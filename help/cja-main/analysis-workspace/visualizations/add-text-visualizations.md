---
title: 向Analysis Workspace项目添加文本可视化图表
description: 了解如何在Customer Journey Analytics的Analysis Workspace项目中添加和配置文本可视化图表。
feature: Visualizations
doc-type: technical video
team: Technical Marketing
kt: 13426
role: User
level: Beginner
last-substantial-update: 2026-02-09T00:00:00Z
exl-id: f4ddd3fb-af0b-4594-9cde-81829885b611
source-git-commit: 0e2ff3688878b5d2e8f3373350608c040333225f
workflow-type: tm+mt
source-wordcount: '76'
ht-degree: 0%

---

# 将[!UICONTROL 文本]可视化图表添加到[!DNL Analysis Workspace]中的[!DNL Customer Journey Analytics]项目

了解如何在Customer Journey Analytics的Analysis Workspace项目中添加和配置文本可视化图表。 当您想要传达与量度有关的疑虑或向项目中的其他数据添加上下文时，此可视化图表非常有用。

>[!VIDEO](https://video.tv.adobe.com/v/3479718/?captions=chi_hans&quality=12&learn=on)

有关[!UICONTROL 文本]可视化的详细信息，请访问[文档](https://experienceleague.adobe.com/docs/analytics-platform/using/cja-workspace/visualizations/text.html?lang=zh-Hans){target="_blank"}。
