---
title: 在Analysis Workspace项目中创建100%栈叠可视化图表
description: 了解如何在Customer Journey Analytics的Analysis Workspace项目中使用百分比而不是原始值来配置完全栈叠的可视化图表。
feature: Visualizations
doc-type: technical video
thumbnail: 23131.jpg
kt: 13434
role: User
level: Beginner
last-substantial-update: 2026-02-09T00:00:00Z
exl-id: 47c482e4-91b2-4c0d-bd6d-719f20b91204
source-git-commit: 0e2ff3688878b5d2e8f3373350608c040333225f
workflow-type: tm+mt
source-wordcount: '70'
ht-degree: 0%

---

# 在Analysis Workspace项目中创建100%栈叠可视化图表

了解如何在Customer Journey Analytics的Analysis Workspace项目中使用百分比而不是原始值来配置完全栈叠的可视化图表。

>[!VIDEO](https://video.tv.adobe.com/v/3479707/?captions=chi_hans&quality=12&learn=on)

有关[!UICONTROL 完全栈叠的变量]可视化的详细信息，请访问[文档](https://experienceleague.adobe.com/docs/analytics-platform/using/cja-workspace/visualizations/bar.html?lang=zh-Hans){target="_blank"}。
