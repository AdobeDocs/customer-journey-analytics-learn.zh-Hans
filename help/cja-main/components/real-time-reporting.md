---
title: Customer Journey Analytics中的实时报表
description: 了解Adobe Customer Journey Analytics中的实时报表如何直接在Analysis Workspace中提供收入、转化和访问等关键量度。
feature: Real-time Reporting
role: Admin, User
level: Beginner
doc-type: Feature Video
duration: 304
last-substantial-update: 2026-02-17T00:00:00Z
jira: KT-20294
source-git-commit: 0985d4e9f64d79575ebde79e97eadbe89145ce38
workflow-type: tm+mt
source-wordcount: '78'
ht-degree: 10%

---

# 实时报告

了解Adobe Customer Journey Analytics中的实时报表如何直接在Analysis Workspace中提供收入、转化和访问等关键量度。 此功能非常适合于主要促销活动、产品发布和历程验证，它使营销人员能够在每一分钟都非常重要时做出更快、数据驱动的决策。

有关更多信息，请阅读该[文档](https://experienceleague.adobe.com/zh-hans/docs/analytics-platform/using/cja-components/real-time-reporting/real-time){target="_blank"}。

>[!VIDEO](https://video.tv.adobe.com/v/3479943/?learn=on&enablevpops)
